    ...
    for element in data.data:
      ...
      if len(self.data) < self.max_size:
        ...
        if random.uniform(0,1)>0.5:
          i = random.radint(0,self.data[i].clone())
          self.data[i] = element
        else:
          to_return.append(element)
...
dataloader = torch.utils.data.DataLoader(
    ...
    #ImageDataset("/content/drive/MyDrive/Colab Notebooks/datasets/%s" %dataset_name, transforms_=transforms_,unaligned=True),
...
val_dataloader = torch.utils.data.DataLoader(
    #how to set data path???
    #ImageDataset("/content/drive/MyDrive/Colab Notebooks/datasets/%s" %dataset_name, transforms_=transforms_, unaligned=True, mode="test"),
    batch_size =5,
    shuffle=True,
    num_workers=1,
)
def sample_images(bathces_done):
  ...
  for i,batch in enumerate(dataloader):
    ...
    sys.stdout.write(
        "/r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, adv: %f, cycle: %f, identity: %f] ETA: %s" %(
            epoch,n_epochs,i,len(dataloader),loss_D.item(),loss_G.item(),loss_GAN.item(),loss_cycle.item(),loss_identity.item(),time_left,
        )
    )
